...
// Get bus amenities
$amenities = [];
$stmt = $conn->prepare("SELECT name FROM bus_amenities WHERE bus_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($name) || empty($departure_location) || empty($arrival_location)) {
    ...
    try {
      // Update bus
      $stmt = $conn->prepare("UPDATE buses SET name = ?, description = ?, departure_location = ?, arrival_location = ?, 
                             departure_time = ?, arrival_time = ?, price = ?, total_seats = ?, bus_type = ? 
                             WHERE id = ?");
      
      $stmt->bind_param("ssssssdssi", $name, $description, $departure_location, $arrival_location, 
                       $departure_time, $arrival_time, $price, $total_seats, $bus_type, $bus_id);
      ...
      if ($stmt->execute()) {
        ...
        $bus['departure_location'] = $departure_location;
        $bus['arrival_location'] = $arrival_location;
...
// For demonstration, create sample data if needed
if (empty($bus)) {
  $bus = [
    'id' => $bus_id,
    'name' => 'Deluxe Express',
...
if (empty($amenities)) {
  $amenities = ['wifi', 'air_conditioning', 'reclining_seats', 'charging_ports'];
}
?>
...
<head>
  ...
  <title>Edit Bus - Bus Operator Dashboard</title>
  ...
  <div class="dashboard">
    ...
    <div class="dashboard-content">
      <div class="dashboard-header">
        <h2>Edit Bus</h2>
        <p>Update details for <?php echo $bus['name']; ?></p>
      </div>
          ...
          <div class="form-section">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" id="name" name="name" class="form-control" value="<?php echo htmlspecialchars($bus['name']); ?>" required>
              ...
              <div class="form-group">
                ...
                <select id="bus_type" name="bus_type" class="form-control" required>
                  <option value="regular" <?php echo $bus['bus_type'] == 'regular' ? 'selected' : ''; ?>>Regular</option>
                  <option value="deluxe" <?php echo $bus['bus_type'] == 'deluxe' ? 'selected' : ''; ?>>Deluxe</option>
                  ...
                  <option value="tourist" <?php echo $bus['bus_type'] == 'tourist' ? 'selected' : ''; ?>>Tourist</option>
                </select>
              </div>
            </div>
            <div class="form-group">
              <label for="description">Bus Description</label>
              <textarea id="description" name="description" class="form-control"><?php echo htmlspecialchars($bus['description']); ?></textarea>
          ...
          <div class="form-section">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" id="departure_location" name="departure_location" class="form-control" value="<?php echo htmlspecialchars($bus['departure_location']); ?>" required>
              </div>
              <div class="form-group">
                <label for="arrival_location">Arrival Location <span class="required">*</span></label>
                <input type="text" id="arrival_location" name="arrival_location" class="form-control" value="<?php echo htmlspecialchars($bus['arrival_location']); ?>" required>
              </div>
            </div>
            <div class="form-row">
              <div class="form-group">
                <label for="departure_time">Departure Time <span class="required">*</span></label>
                <input type="time" id="departure_time" name="departure_time" class="form-control" value="<?php echo $bus['departure_time']; ?>" required>
              </div>
              <div class="form-group">
                <label for="arrival_time">Arrival Time <span class="required">*</span></label>
                <input type="time" id="arrival_time" name="arrival_time" class="form-control" value="<?php echo $bus['arrival_time']; ?>" required>
              </div>
            </div>
          </div>
          ...
          <div class="form-section">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="number" id="total_seats" name="total_seats" class="form-control" value="<?php echo $bus['total_seats']; ?>" min="1" required>
                <div class="form-help">Changing this may require updating your seat configuration</div>
              </div>
              <div class="form-group">
                <label for="price">Price per Seat ($) <span class="required">*</span></label>
                <input type="number" id="price" name="price" class="form-control" value="<?php echo $bus['price']; ?>" step="0.01" min="0" required>
              </div>
            </div>
          </div>
          ...
          <div class="form-section">
            ...
            <div class="amenities-list">
              <div class="amenity-item">
                <input type="checkbox" id="amenity-wifi" name="amenities[]" value="wifi" <?php echo in_array('wifi', $amenities) ? 'checked' : ''; ?>>
                <label for="amenity-wifi">WiFi</label>
              </div>
              <div class="amenity-item">
                <input type="checkbox" id="amenity-ac" name="amenities[]" value="air_conditioning" <?php echo in_array('air_conditioning', $amenities) ? 'checked' : ''; ?>>
                <label for="amenity-ac">Air Conditioning</label>
              </div>
              <div class="amenity-item">
                <input type="checkbox" id="amenity-charging" name="amenities[]" value="charging_ports" <?php echo in_array('charging_ports', $amenities) ? 'checked' : ''; ?>>
                <label for="amenity-charging">Charging Ports</label>
              </div>
              <div class="amenity-item">
                <input type="checkbox" id="amenity-tv" name="amenities[]" value="tv" <?php echo in_array('tv', $amenities) ? 'checked' : ''; ?>>
                <label for="amenity-tv">TV/Entertainment</label>
              </div>
              <div class="amenity-item">
                <input type="checkbox" id="amenity-refreshments" name="amenities[]" value="refreshments" <?php echo in_array('refreshments', $amenities) ? 'checked' : ''; ?>>
                <label for="amenity-refreshments">Refreshments</label>
              </div>
              <div class="amenity-item">
                <input type="checkbox" id="amenity-reclining" name="amenities[]" value="reclining_seats" <?php echo in_array('reclining_seats', $amenities) ? 'checked' : ''; ?>>
                <label for="amenity-reclining">Reclining Seats</label>
              </div>
              <div class="amenity-item">
                <input type="checkbox" id="amenity-restroom" name="amenities[]" value="restroom" <?php echo in_array('restroom', $amenities) ? 'checked' : ''; ?>>
                <label for="amenity-restroom">Restroom</label>
              </div>
              <div class="amenity-item">
                <input type="checkbox" id="amenity-blankets" name="amenities[]" value="blankets" <?php echo in_array('blankets', $amenities) ? 'checked' : ''; ?>>
