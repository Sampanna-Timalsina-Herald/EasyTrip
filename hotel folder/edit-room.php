...
$stmt = $conn->prepare("SELECT r.*, h.name as hotel_name 
                       FROM hotel_rooms r 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($room_number)) {
    ...
    if ($result->num_rows === 0) {
      $error_message = 'You do not have permission to edit rooms for this hotel.';
    } else {
      // Check if room number already exists for this hotel (excluding current room)
      $stmt = $conn->prepare("SELECT id FROM hotel_rooms WHERE hotel_id = ? AND room_number = ? AND id != ?");
      $stmt->bind_param("isi", $hotel_id, $room_number, $room_id);
      $stmt->execute();
      $result = $stmt->get_result();
      ...
      if ($result->num_rows > 0) {
        ...
        try {
          // Update room
          $stmt = $conn->prepare("UPDATE hotel_rooms SET hotel_id = ?, room_number = ?, room_type = ?, price_per_night = ?, status = ?, description = ? WHERE id = ?");
          $stmt->bind_param("issdssi", $hotel_id, $room_number, $room_type, $price_per_night, $status, $description, $room_id);
          $stmt->execute();
          ...
          // Get updated hotel name
          foreach ($hotels as $h) {
            if ($h['id'] == $hotel_id) {
              $room['hotel_name'] = $h['name'];
              break;
            }
          }
        } catch (Exception $e) {
...
if (empty($room)) {
  $room = [
    ...
    'status' => 'available',
    'description' => 'Comfortable double room with mountain view.'
  ];
}

if (empty($amenities)) {
  $amenities = ['wifi', 'air_conditioning', 'tv', 'private_bathroom'];
}
?>
...
<head>
  ...
  <title>Edit Room - Hotel Owner Dashboard</title>
  ...
  <div class="dashboard">
    ...
    <div class="dashboard-content">
      <div class="dashboard-header">
        <h2>Edit Room</h2>
        <p>Update room details for <?php echo $room['hotel_name']; ?></p>
      </div>
        ...
        <form action="" method="POST">
          <div class="form-section">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <select id="hotel_id" name="hotel_id" class="form-control" required>
                  <?php foreach ($hotels as $hotel): ?>
                    <option value="<?php echo $hotel['id']; ?>" <?php echo $room['hotel_id'] == $hotel['id'] ? 'selected' : ''; ?>>
                      <?php echo $hotel['name']; ?>
                    </option>
                  <?php endforeach; ?>
                </select>
              ...
              <div class="form-group">
                ...
                <input type="text" id="room_number" name="room_number" class="form-control" value="<?php echo $room['room_number']; ?>" required>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <select id="room_type" name="room_type" class="form-control" required>
                  ...
                  <option value="twin" <?php echo $room['room_type'] == 'twin' ? 'selected' : ''; ?>>Twin</option>
              ...
              <div class="form-group">
                ...
                <input type="number" id="price_per_night" name="price_per_night" class="form-control" value="<?php echo $room['price_per_night']; ?>" step="0.01" min="0" required>
              </div>
            </div>
            <div class="form-group">
          ...
          <div class="form-section">
            ...
            <div class="form-group">
              ...
              <textarea id="description" name="description" class="form-control"><?php echo $room['description']; ?></textarea>
            ...
            <div class="form-group">
              ...
              <div class="amenities-list">
                ...
                <div class="amenity-item">
                  <input type="checkbox" id="amenity-fridge" name="amenities[]" value="refrigerator" <?php echo in_array('refrigerator', $amenities) ? 'checked' : ''; ?>>
                  <label for="amenity-fridge">Refrigerator</label>
                </div>
                <div class="amenity-item">
                  <input type="checkbox" id="amenity-balcony" name="amenities[]" value="balcony" <?php echo in_array('balcony', $amenities) ? 'checked' : ''; ?>>
                  <label for="amenity-balcony">Balcony</label>
                </div>
                <div class="amenity-item">
                  <input type="checkbox" id="amenity-private-bathroom" name="amenities[]" value="private_bathroom" <?php echo in_array('private_bathroom', $amenities) ? 'checked' : ''; ?>>
                  <label for="amenity-private-bathroom">Private Bathroom</label>
                </div>
                <div class="amenity-item">
                  <input type="checkbox" id="amenity-hot-water" name="amenities[]" value="hot_water" <?php echo in_array('hot_water', $amenities) ? 'checked' : ''; ?>>
                  <label for="amenity-hot-water">Hot Water</label>
                </div>
                <div class="amenity-item">
                  <input type="checkbox" id="amenity-mountain-view" name="amenities[]" value="mountain_view" <?php echo in_array('mountain_view', $amenities) ? 'checked' : ''; ?>>
